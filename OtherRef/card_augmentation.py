...
def augment_colour_stress(card_path: str,
                           out_size: tuple = None):
    card   = _load_card(card_path)
    h, w   = card.shape[:2]
    target = out_size or (w, h)
...
def process_dataset(input_dir: str,
    ...
    for class_dir in class_dirs:
        ...
        print(f"[{class_dir.name}]  {len(imgs)} source images x 5 augmentations …")
        ...
        print(f"  -> {len(imgs) * 5} images written to {out_cls}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        # Single-card preview - saves 5 augmented images next to the source.
        src     = sys.argv[1]
        stem    = Path(src).stem
        ...
        print("Usage:")
        print("  Single card preview : python card_augmentation_2.py card.png")
        print("  Full dataset        : python card_augmentation_2.py input_dir/ output_dir/")